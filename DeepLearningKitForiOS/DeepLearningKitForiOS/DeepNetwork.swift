...
open class DeepNetwork {
    ...
    open func classify(_ image: [Float], shape:[Float]) -> Int {
        ...
        NSLog(lastLayerName)
        let data = Data(bytesNoCopy: lastMetalBuffer.contents().assumingMemoryBound(to: UInt8.self),
                        count: classification_results.count * MemoryLayout<Float>.size, deallocator: .none)
        
        (data as NSData).getBytes(&classification_results, length:(Int(classification_results.count)) * MemoryLayout<Float>.size)
